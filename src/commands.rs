...
pub async fn register_commands(ctx: &Context) -> serenity::Result<()> {
    let commands = vec![CreateCommand::new("setup")
        .description(
            "Configure safe and avoid roles for automatic restoration and set the log channel",
        )
...
async fn get_bot_accessible_text_channels(
    ctx: &Context,
...
async fn get_bot_accessible_text_channels_with_default(
    ctx: &Context,
    guild_id: serenity::all::GuildId,
    default_channel_id: Option<u64>,
) -> Vec<CreateSelectMenuOption> {
    // Get bot ID and prepare permissions check info from cache
    let bot_id = ctx.cache.current_user().id;
    let guild_info = ctx.cache.guild(guild_id).map(|guild| {
        let bot_member = guild.members.get(&bot_id).cloned();
        (guild.clone(), bot_member)
    });
    ...
    let (guild, bot_member) = match guild_info {
        Some((g, Some(m))) => (g, m),
        _ => return vec![],
    };
    ...
    channels
        ...
        .filter_map(|ch| {
            ...
            // Check bot permissions in this channel using the approved method
            let perms = guild.user_permissions_in(ch, &bot_member);
...
pub async fn handle_setup_command(
    ...
    let channel_options =
        get_bot_accessible_text_channels_with_default(ctx, guild_id, log_channel).await;
    ...
    let admin_filter_status = if filter_admin_roles {
        "✅ Enabled"
    } else {
        "❌ Disabled"
    };
...
pub async fn handle_admin_filter_toggle(
    ...
    match db.set_filter_admin_roles(guild_id.get(), new_filter) {
        Ok(_) => {
            ...
            let safe_roles = db.get_safe_roles(guild_id.get()).unwrap_or_default();
            let safe_role_mentions: Vec<String> =
                safe_roles.iter().map(|id| format!("<@&{}>", id)).collect();

            let avoid_roles = db.get_avoid_roles(guild_id.get()).unwrap_or_default();
            let avoid_role_mentions: Vec<String> =
                avoid_roles.iter().map(|id| format!("<@&{}>", id)).collect();
            ...
            if !new_filter {
                updated_embed = updated_embed.field(
                    "Security Rules",
                    "• Roles with **Admin** permission won't be auto-restored",
                    false,
                );
            ...
            let safe_roles_set: std::collections::HashSet<u64> =
                safe_roles.iter().cloned().collect();
            let avoid_roles_set: std::collections::HashSet<u64> =
                avoid_roles.iter().cloned().collect();
                ...
                .map(|g| {
                    g.roles
                        ...
                        .map(|r| {
                            ...
                            let mut option =
                                CreateSelectMenuOption::new(r.name.clone(), role_id_str);
            ...
            let admin_filter_status = if new_filter {
                "✅ Enabled"
            } else {
                "❌ Disabled"
            };
            let admin_filter_button =
                CreateButton::new(format!("admin_filter_toggle_{}", _user_id_str))
                    .label(format!("Re-assign Admin Roles: {}", admin_filter_status))
                    .style(if new_filter {
                        ButtonStyle::Success
                    } else {
...
pub async fn handle_safe_roles_select(
    ...
    match db.set_safe_roles(guild_id.get(), &role_ids) {
        ...
        .field("Log Channel", log_channel_display, false);
            ...
            let admin_filter_button =
                CreateButton::new(format!("admin_filter_toggle_{}", user_id_str))
                    .label(format!("Re-assign Admin Roles: {}", admin_filter_status))
                    .style(if filter_admin_roles {
                        ButtonStyle::Success
                    } else {
...
pub async fn handle_log_channel_select(
    ...
    let channel_id = match &interaction.data.kind {
        ...
        serenity::all::ComponentInteractionDataKind::StringSelect { values } => {
            ...
            values
                .first()
                .and_then(|ch_id_str| ch_id_str.parse::<u64>().ok())
        }
        _ => None,
    };
